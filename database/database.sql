...
-- _ table ? => 엑셀에서 sheet
-- _ column ? => 엑셀에서 열 이름(A,B,C ...)
-- _ 자료형 ? => 저장 할 데이터의 종류
-- # int? => 숫자 저장을 위한 자료형
-- # varchar? => 문자열 저장을 위한 자료형
-- _ query ? => 데이터 베이스에 요청 하는 명령어  
-- _ CRUD ? => : Create(생성), Read(읽기), Update(갱신), Delete(삭제)를 묶어서 일컫는 말
-- _ work bench 명령어
...
-- _ 아래 명령의 의미는 bot_test 라는 데이터 베이스 안에 있는 class1 이라는 테이블의 모든 정보를 보여줘
select _ from bot_test1.class1;
